...
class Command(BaseCommand):
    ...
    def run_notebookapp(self, initialize, options, use_kernel_specs=True):
    ...
    def get_notebook(self, options):
        ...
        def run_notebook():
            self.run_notebookapp(initialize, options, use_kernel_specs)
        return run_notebook
    ...
    def get_jupyterlab(self, options):
        ...
        if not NotebookApp or not issubclass(LabApp, NotebookApp):
            initialize = LabApp.initialize_server
        else:
            def initialize(*args, **kwargs):
                app = LabApp.instance()
                app.initialize(*args, **kwargs)
        ...
        def run_jupyterlab():
            self.run_notebookapp(initialize, options)
